...
// 初期データを投入
initializeMockData();
export const fetchCarePlans = async (userId) => {
    try {
...
export const addCarePlan = async (planData) => {
    try {
        await new Promise(resolve => setTimeout(resolve, 300));
        const newPlan = {
            id: `cp_${Date.now()}`,
 ...
 * ケアプランの進捗を更新
 */
export const updateCarePlanProgress = async (planId, progress) => {
    try {
        await new Promise(resolve => setTimeout(resolve, 300));
        const currentPlan = carePlans.get(planId);
        if (!currentPlan) {
 ...
 * ケアプランの目標を更新
 */
export const updateCarePlanGoal = async (planId, goalId, goalUpdates) => {
    try {
        await new Promise(resolve => setTimeout(resolve, 300));
        const currentPlan = carePlans.get(planId);
        if (!currentPlan) {
        ...
        const updatedGoals = currentPlan.goals.map(goal => goal.id === goalId ? { ...goal, ...goalUpdates } : goal);
        const updatedPlan = {
            ...currentPlan,
            goals: updatedGoals,
            updatedAt: new Date().toISOString(),
 ...
 * ケアプランのサービスを更新
 */
export const updateCarePlanService = async (planId, serviceId, serviceUpdates) => {
    try {
        await new Promise(resolve => setTimeout(resolve, 300));
        const currentPlan = carePlans.get(planId);
        if (!currentPlan) {
        ...
        const updatedServices = currentPlan.services.map(service => service.id === serviceId ? { ...service, ...serviceUpdates } : service);
        const updatedPlan = {
            ...currentPlan,
